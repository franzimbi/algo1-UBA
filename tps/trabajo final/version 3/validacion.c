...
static bool crop(argumentos_t *argumentos, char* crop){
    ...
    argumentos->crop.ancho[argumentos->crop.cantidad_recortes] = strtol(crop, &endptr,10);
    if (*endptr != 'x' || *endptr != 'X')
    {
        return false; 
    }

    argumentos->crop.alto[argumentos->crop.cantidad_recortes] = strtol(endptr + 1, &endptr ,10);
    if (*endptr != '+')
    {
        return false; 
    }
    ...
    argumentos->crop.x[argumentos->crop.cantidad_recortes] = strtol(endptr + 1, &endptr ,10);
    if (*endptr != '+')
    {
        return false;
    }

    argumentos->crop.y[argumentos->crop.cantidad_recortes] = strtol(endptr + 1, &endptr ,10);
    if (*endptr != '\0')
    {
        return false; 
    }

    argumentos->crop.recortes_realizados = 0;
    argumentos->crop.cantidad_recortes++;
