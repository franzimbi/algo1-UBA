...
const struct tabla_de_filtros tabla_filtro[] = {
...
bool cadena_a_filtro(const char *nombre, filtro_t *filtro){
    for(size_t i = 0; i < sizeof(tabla_filtro) / sizeof(tabla_filtro[0]); i++)
        if(! strcmp(tabla_filtro[i].nombre, nombre)){
            *filtro= i;
            return true;
        }
    return false;
...
int numero_de_parametros(filtro_t filtro){
    return tabla_filtro[filtro].parametros;
}
...
void aplicar_filtro(imagen_t *imagen, filtro_t filtro, int parametro){
        for(int x = 0; x < imagen->alto; x++){	 
            for (int y = 0; y < imagen->ancho; y++){
                imagen->lienzo[x][y] = (tabla_filtro[filtro]).funcion(imagen->lienzo[x][y], parametro);
            }
        }
}
